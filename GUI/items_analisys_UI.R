items_query_UI = function(input,output){
  output$disease_couple = renderUI({
    selectizeInput(
      'disease_couple', label = "Select the item to start the analysis", choices = c(disease,nano,drugs,chemical), multiple = FALSE,
      options = list(create = TRUE)
    )
  })
  
  output$th_slider_couple = renderUI({
    sliderInput("th_slider_couple", label = "Strenght of Similarity",
                min = 0, max = 99, value = 99,step=1)
  })  
  
  output$Go_couple = renderUI({
    actionButton(inputId = "Go_couple",label  = "Start",icon("circle-arrow-right", lib = "glyphicon"))
  })
  
  output$refresh_couple = renderUI({
    actionButton(inputId = "refresh_couple", label  = "Refresh",icon("refresh", lib = "glyphicon"))
  })
}
