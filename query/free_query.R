free_query_size=function(input,output,disease_list,selected_nodes,ADJ,ADJ01,ADJ01_RANK,th_p = input$th_slider/100,node_type,chemMat,join10,g,g_geni2,items_list){
  ...
  nano_ADJ_S = colnames(ADJ_S)[colnames(ADJ_S) %in% nano]
  drug_ADJ_S = colnames(ADJ_S)[colnames(ADJ_S) %in% drugs]
  dis_ADJ_S = colnames(ADJ_S)[colnames(ADJ_S) %in% disease]
  chem_ADJ_S = colnames(ADJ_S)[colnames(ADJ_S) %in% chemical]

  
  output$querySize <- renderUI({HTML(paste(
        "Submatrix Dimensions: ",dim(ADJ_S),"\n",
        "Number of nanos: ",length(nano_ADJ_S) , "\n",
        "Number of drugs: ",length(drug_ADJ_S) , "\n",
        "Number of diseases: ",length(dis_ADJ_S) , "\n",
        "Number of chemicals:",length(chem_ADJ_S) ,"\n",sep=""
  ))}) 
  
}

free_query2 = function(input,output,disease_list,selected_nodes,ADJ,ADJ01,ADJ01_RANK,th_p = input$th_slider/100,node_type,chemMat,join10,g,g_geni2,items_list){
